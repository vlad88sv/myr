...
$c = 'SELECT t2.usuario AS "Naviera", `codigo_contenedor` AS "N° contenedor", `tipo_contenedor` AS "Tipo", estado AS "Estado", `referencia_papel` AS "EIR", `num_bl` AS "B/L", `codigo_cheque` AS "Cheque", CONCAT(\'<a href="/elaboracion.de.condicion.html?ID=\', ID_condicion, \'">\', `fecha_ingreso`, "</a>") AS "Fecha" FROM `opsal_condiciones` AS t1 LEFT JOIN opsal_usuarios AS t2 ON t1.codigo_agencia = t2.codigo_usuario WHERE 1 ' . $WHERE . ' ORDER BY  `codigo_contenedor` ASC';
$r = db_consultar($c);
