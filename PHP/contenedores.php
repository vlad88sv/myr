...
if (S_iniciado() && _F_usuario_cache('nivel') == 'agencia') {
    ...
    $menu[] = array('url' => PROY_URL . 'contenedores.html', 'modo' => 'doble.movimiento', 'titulo' => 'DOBLE MOVIMIENTO');
    $menu[] = array('url' => PROY_URL . 'contenedores.html', 'modo' => 'remocion.bloque', 'titulo' => 'REMOCION BLOQUE');
    $menu[] = array('url' => PROY_URL . 'contenedores.html', 'modo' => 'salida', 'titulo' => 'DESPACHO');
    $menu[] = array('url' => PROY_URL . 'contenedores.html', 'modo' => 'salida.bloque', 'titulo' => 'DESPACHO BLOQUE');
    ...
    <tbody>
        <tr>
            ...
            <td id="opsal_mapa" style="width:755px;background-color:white;text-align:left;vertical-align: top;">
                <?php
                $c = 'SELECT * FROM `virt_patios`';
                $r = db_consultar($c);

                $patios = '';
                while ($f = mysqli_fetch_assoc($r)) {
                    $patios .= '<option value="' . $f['id'] . '">' . $f['nombre'] . '</option>';
                }
                ?>
                <p style="text-align: center;">Patio: <select id='codigo_patio'><?php echo $patios; ?></select></p>
                <hr />
...
<script type="text/javascript">
    ...
    function iniciar_mapa(opciones) {
        ...
        opciones = typeof opciones !== 'undefined' ? opciones : {codigo_patio: $("#codigo_patio").val()};
