...
if (!empty($_GET['ID']) && is_numeric($_GET['ID']))
{
    ...
    $c = 'SELECT `codigo_agencia`, `codigo_contenedor`, `tipo_contenedor`, `referencia_papel`, `codigo_cheque`, `fecha_ingreso`, `estado`, `num_bl` FROM `opsal_condiciones` WHERE `ID_condicion` = ' . $id_condicion;
    $r = db_consultar($c);
    $f = db_fetch($r);   
}
...
{
    $DATOS = array_intersect_key($_POST,array_flip(array('ID_condicion', 'codigo_agencia', 'codigo_contenedor', 'codigo_cheque', 'referencia_papel','fecha_ingreso','tipo_contenedor','estado','num_bl')));
...
<table id="opsal_ims">
    <tbody>
        ...
        <tr>
            <td>Número B/L</td><td><input type="text" id="referencia_papel" name="num_bl" value="<?php echo @$f['num_bl']; ?>" /></td>
        </tr>
        <tr>
