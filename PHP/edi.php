...
function edi_log($codigo_orden_edi, $comentario) {
    $DATOS['codigo_orden_edi'] = $codigo_orden_edi;
    $DATOS['fechatiempo'] = mysql_datetime();
    $DATOS['comentario'] = $comentario;
    db_agregar_datos('edi_log', $DATOS);
}
...
function crear_EDI($codigo_orden) {
    ...
    if ($EDI['activo'] == '0') {
        // 2 = se omitio en el momento de la ejecucion porque no se necesitaba EDI
        $EDI_ESTADO = 9;
    }
    ...
    if ($EDI_ESTADO === 0) {
        ...
        if ($IO == 3) // import
            $SEAL = $DATOS['ingreso_marchamo'];
        else
            $SEAL = $DATOS['egreso_marchamo'];
    ...
    if ( $EDI_ESTADO === 0 ) {
        ...
        edi_log($codigo_orden_edi, 'EDI file not added to the queue - EDI inactive for this agency.');
    }
